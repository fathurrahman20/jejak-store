...
export default async function Navbar() {
  ...
  return (
    // CHANGE: Tambahkan 'relative' agar dropdown mobile menu posisinya pas
    <nav className="relative container max-w-[1130px] mx-auto flex items-center justify-between bg-[#3A4F41] p-4 md:p-5 rounded-3xl px-4 md:px-5 xl:px-5">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        {/* CHANGE: Mobile Menu Button (Hamburger) - Muncul di Mobile Only */}
        <MobileMenu />
        ...
        {session && user.role === "CUSTOMER" ? (
          <>
            <p className="text-white hidden sm:block ml-2">Hi, {user.name}</p>

            <Menu as="div" className="relative ml-2 md:ml-4 shrink-0">
              <div>
                <MenuButton className="relative flex text-sm bg-white rounded-full ring-2 ring-[#111827] focus:outline-none hover:ring-2 hover:ring-[#2a3e6b] focus:ring-offset-2">
                  ...
                  <div className="w-10 h-10 md:w-[48px] md:h-[48px] flex shrink-0 rounded-full p-1 border overflow-hidden">
                    <img
          ...
          <div className="flex gap-2">
            <Link
              href="/sign-in"
              className="p-[8px_16px] md:p-[12px_20px] bg-white rounded-full font-semibold text-sm md:text-base whitespace-nowrap"
            >
              Sign In
            </Link>
            <Link
              href="/sign-up"
              className="p-[8px_16px] md:p-[12px_20px] bg-white rounded-full font-semibold text-sm md:text-base hidden sm:block"
            >
              Sign Up
            </Link>
          </div>
